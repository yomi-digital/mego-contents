          ...
          <div v-for="key in Object.keys(content)" :key="key" class="body my-4">
            <template v-if="key !== 'name' && key !== 'title' && key !== 'category' && key !== 'timestamp'">
              ...
              <p v-html="content[key]"
                v-if="key !== 'image' && content[key][0] && content[key][0].indexOf('ipfs') === -1 && !isArray(content[key])"></p>
              <!--Tag-->
              <div style="display:flex;margin-top: 1rem;" v-if="isArray(content[key]) && content[key][0].indexOf('ipfs') === -1">
